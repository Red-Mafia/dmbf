#!/usr/bin/python3
#-*-coding:utf-8-*-
# Made With ❤️ RED-MAFIA AND KARMA
# Update V0.1

# Copyright© RED-MAFIA AND KARMA
# 100% Open Source Code

# Author : RED-MAFIA AND KARMA
# Facebook (RED MAFIA)   : https://www.facebook.com/R.k.Riazkhan.termux.tool.owner
# Instagram (rk.riazkhan143)    : rk.riazkhan143
# Whatsapp (+923188214452)      : +234 811 004 4418
# YouTube (RED-MAFIA)           : https://youtube.com/channel/UCPOWtzNan9ff-E0RI66Wu2A
#RED-MAFIA AND KARMA TEEM
...
_logo_line_3_ = ('%s| |) | |\/| | _ \ _|  %s│   %s• Code By RK AND KARMA  %s│'%(_P_,_U_,_P_,_U_))
_logo_line_4_ = ('%s|___/|_|  |_|___/_|   %s│ %sGithub.com/Red-Mafia/dmbf %s│'%(_P_,_U_,_P_,_U_))
_logo_line_5_ = ('%s RK AND KARMA Team 2021    └─────────────────────────┘'%(_U_))
def _my_logo_():
...
_oscylopsce_ = '__RED-MAFIA__'
_ascylapsci_ = '__KARMA__'
_escylipsce_ = '__Dapunta_Love_KARMA__'
_uscylupsci_ = '__My_Love____RED-MAFIA____RED-MAFIA_Love_KARMA____KARMA____Forever__'
...
### Menu
def _menu_dev_(_Red-Mafia_Ganteng_Banget_):
    _clear_()
    _my_logo_()
    if _uscylupsci_ not in _Red-Mafia_Ganteng_Banget_:_dapunta_cici_('%s[%s!%s] %sHayoo Mau Recode Ya?'%(_M_,_P_,_M_,_P_))
    else:pass
    try:
        ...
        _bersih_()
        time.sleep(2)
        _login_dev_(_Red-Mafia_Ganteng_Banget_)
    except requests.exceptions.ConnectionError:
    ...
    if _dapunta_menu__cici_dapunta__ in ['',' ']:
        ...
        time.sleep(2)
        _menu_dev_(_Red-Mafia_Ganteng_Banget_)
        ...
        _ugen_dev_(_Red-Mafia_Ganteng_Banget_)
...
def _publik_dev_(_tok_dev_):
    _Red-Mafia_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
    ...
    try:
        ...
        time.sleep(2)
        _menu_dev_(_Red-Mafia_Sayang_Cici_)
    try:
        ...
        for _Red-Mafia_Cici_Forever_ in _lo_dev_["data"]:
            try:
                _id_tampung_.append(_Red-Mafia_Cici_Forever_["id"]+"•"+_Dapunta_Cici_Forever_["name"])
                _jso_exec_.write(_Red-Mafia_Cici_Forever_["id"]+"•"+_Dapunta_Cici_Forever_["name"]+"\n")
            except:continue
        _jso_exec_.close()
...
class _crack_dev_:
    def __init__(self,_files_dapunta_):
        self._Red-Mafia_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
        self._ok_dapunta_ = []
        self._cp_dapunta_ = []
        ...
        while True:
            ...
            if _pilih_pass_=="":
                _dapunta_cici_('%s[%s!%s] %sIsi Yang Benar'%(_M_,_P_,_M_,_P_))
                time.sleep(2)
                _menu_dev_(self._Red-Mafia_Sayang_Cici_)
